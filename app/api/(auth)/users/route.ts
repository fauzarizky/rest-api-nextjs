...
export const PATCH = async (request: Request) => {
  try {
    ...
    if (!userId || !newUsername) {
      return new NextResponse(JSON.stringify({ message: "userId and newUsername are required" }), { status: 400 });
    }
...
export const DELETE = async (request: Request) => {
  try {
    ...
    if (!userId) {
      return new NextResponse(JSON.stringify({ message: "userId are required" }), { status: 400 });
    }
